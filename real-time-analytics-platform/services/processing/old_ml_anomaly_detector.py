...
VALUES
(:alert_id, :source, :key, :event_id, :event_ts, :window_start, :window_end,
 :metric_name, :actual_value, :expected_value, :score, :severity, :anomaly_type, CAST(:details_json AS jsonb),
 :method, :model_version)
...
def make_features(kdf: pd.DataFrame) -> pd.DataFrame:
    """
    Build robust time-series features from raw values.
    kdf must be sorted by event_ts.
    """
    ...
    feats["z_approx"] = (s - feats["roll_mean_10"]) / (feats["roll_std_10"].replace(0, np.nan))
    feats["z_approx"] = feats["z_approx"].replace([np.inf, -np.inf], np.nan).fillna(0.0)

    # fill NaNs
    feats = feats.fillna(0.0)
    return feats

# Root Cause / Alert Explaination
def generate_explanation(features_row):
    ...
    if features_row["roll_std_10"] > features_row["value"] * 0.05:
...
def detect_source(source: str, metric_name: str, contamination: float = 0.01):
    ...
    for key, kdf in df.groupby("key", sort=False):
        ...
        # Need enough data to train reasonably
        if len(kdf) < 80:
            continue

        X = make_features(kdf)

        # Train on earlier chunk, score recent chunk
        train = X.iloc[:-20]
        test = X.iloc[-20:]
        ...
        # Threshold: pick top anomalies in the test chunk
        # Use percentile to adapt across keys
        thresh = np.percentile(anomaly_score, 98)

        for i, row in test.iterrows():
            s = float(anomaly_score[list(test.index).index(i)])

            if s < thresh:
                continue

            severity = max(0.0, min(1.0, float(s) * 2))
            explanation = generate_explanation(X.loc[i])
            evt = kdf.iloc[i]
            event_ts = evt["event_ts"].to_pydatetime()

            # Dedupe for same key/method/metric within 2 minutes
            with ENGINE.connect() as conn:
                exists = conn.execute(
            ...
            # Use roll_mean_10 as expected baseline
            expected = float(X.loc[i, "roll_mean_10"])

            # classify spike/drop using delta sign
            anomaly_type = "spike" if float(X.loc[i, "delta"]) > 0 else "drop"

            # Root Cause Explaination Update
            details = {
